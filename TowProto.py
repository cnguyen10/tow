...
def euclidean_distance(matrixN: torch.Tensor, matrixM: torch.Tensor) -> torch.Tensor:
    ...
    # NEW in PyTorch 1.10
    return torch.cdist(x1=matrixN, x2=matrixM, p=2)
...
def dict2data(data_dict: typing.Dict[str, torch.Tensor], device: torch.device) -> typing.Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
    x_t = data_dict['x_t'].to(device)
    y_t = data_dict['y_t'].to(device)
    x_v = data_dict['x_v'].to(device)
    y_v = data_dict['y_v'].to(device)
...
class Tow(object):
    ...
    def initialize_model(self, eps_dataloader: torch.utils.data.DataLoader) -> None:
        ...
        if self.config['network_architecture'] == 'CNN':
            ...
            base_net = MiniCNN(dim_output=None, bn_affine=self.config['batchnorm'])
        else:
    ...
    def embedding_prototypes(self, params: typing.List[torch.Tensor], x_t: torch.Tensor, y_t: torch.Tensor) -> typing.List[torch.Tensor]:
        """Adapt and predict the labels of the queried data
        """
        z_t = self.f_base_net.forward(x_t, params=params) # embed data into the latent space
    ...
    def prediction_one_task(self, param_vec: torch.Tensor, eps_split_data: typing.Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        x_t, y_t, x_v, _ = dict2data(data_dict=eps_split_data, device=param_vec.device)

        # class-based prototypes
        cls_prototypes = self.embedding_prototypes(params=params, x_t=x_t, y_t=y_t)

        z_v = self.f_base_net.forward(x_v, params=params)
        distance_matrix = euclidean_distance(matrixN=z_v, matrixM=cls_prototypes)

        # predition
        y_predict = -distance_matrix # (v_shot * num_classes, num_classes)
    ...
    def validation_loss(self, param_vec: torch.Tensor, minibatch: typing.List[typing.Dict[str, torch.Tensor]], eps_weight: typing.Union[np.ndarray, torch.Tensor]) -> torch.Tensor:
        ...
        val_loss = torch.empty(len(minibatch), dtype=torch.float, device=self.config['device'])
        ...
        for i, task_data in enumerate(minibatch):
            # get training and validation subsets of each task
            x_t, y_t, x_v, y_v = dict2data(data_dict=task_data, device=param_vec.device)
            ...
            # predition
            y_logits = -distance_matrix # (v_shot * num_classes, num_classes)
    ...
    def train_one_minibatch_Adam(self, params: typing.List[torch.Tensor], optimizer_sd: dict, minibatch: typing.List[typing.Dict[str, torch.Tensor]], eps_weight: typing.Union[np.ndarray, torch.Tensor, typing.List[float]]) -> typing.Tuple[torch.Tensor, float, dict]:
        """
        """
        loss_monitor = 0.
        ...
        for i, task_data in enumerate(minibatch):
            # get training and validation subsets of each task
            x_t, y_t, x_v, y_v = dict2data(data_dict=task_data, device=self.config['device'])

            # class-based prototypes
            cls_prototypes = self.embedding_prototypes(params=params, x_t=x_t, y_t=y_t)

            # embedding of the queried data
            z_v = self.f_base_net.forward(x_v, params=params)
            distance_matrix = euclidean_distance(matrixN=z_v, matrixM=cls_prototypes)
            y_logits = -distance_matrix
        ...
        return params, loss_monitor, new_optimizer_sd
    ...
    def evaluate_one_minibatch(self, params: typing.List[torch.Tensor], minibatch: typing.List[typing.Dict[str, torch.Tensor]]) -> typing.Tuple[typing.List[float], typing.List[float]]:
        ...
        for i, minibatch_data in enumerate(minibatch):
            # move data to GPU (if there is a GPU)
            x_t, y_t, x_v, y_v = dict2data(data_dict=minibatch_data, device=self.config['device'])
    ...
    def gauss_newton_components(self, x: torch.Tensor, task_data: typing.Dict[str, torch.Tensor]) -> typing.Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        jacobian_f = torch.empty(size=(self.config['v_shot'] * self.config['num_ways'], self.config['num_ways'], self.num_params), device=self.config['device']) # (num_total_samples, num_classes, X)
        ...
        for _ in range(0, self.config['v_shot'], 1):
            eps_split_data = {
                'x_t': task_data['x_t'],
                'y_t': task_data['y_t'],
                'x_v': task_data['x_v'][pointer:(pointer + self.config['num_ways'])],
                'y_v': task_data['y_v'][pointer:(pointer + self.config['num_ways'])]
            }
            jacobian_f[pointer:(pointer + self.config['num_ways']), :, :] = torch.autograd.functional.jacobian(
    ...
    def jacobian_gauss_newton_matrices(self, xhat_t: torch.Tensor, minibatch: typing.List[typing.Dict[str, torch.Tensor]]) -> typing.Tuple[torch.Tensor, torch.Tensor]:
        ...
        Returns:
            jacobian: the Jacobian matrix where each column is the gradient of each task w.r.t. x
            gauss_newton_diagonals: a tensor/list of Gauss-Newton diagonals
        """
        # a diagonal matrix with small values in the diagonal for numerical stability
        eps_diagonal = 1e-6 * torch.eye(n=self.config['num_ways'], device=xhat_t.device)
        ...
        jacobian = torch.empty(size=(self.config['minibatch'], self.num_params), device=self.config['device']) # (U, X)

        # initialize the diagonal of Gauss-Newton matrix
        gauss_newton_diagonals = torch.empty(size=(self.config['minibatch'], self.num_params), device=self.config['device']) # (U, X)

        for task_id, task_data in enumerate(minibatch): # for each task in minibatch t-th
            jacobian_f, hessian_sigma, sm_outputs = self.gauss_newton_components(x=xhat_t, task_data=task_data) # jacobian_f = (num_samples, C, X)

            # add epsilon to diagonal to prevent error
            L_sigma, _ = torch.linalg.cholesky_ex(input=hessian_sigma + eps_diagonal) # (num_samples, C, C)

            # getting labels
            y_v = task_data['y_v'].to(self.config['device'])
            # convert int labels into one-hot encoding
            y_v = torch.nn.functional.one_hot(input=y_v, num_classes=self.config['num_ways']) # (num_samples, num_classes)

            # Jacobian vector for one task
            dy_v = sm_outputs - y_v # (num_samples, num_classes)
            jacobian[task_id, :] = torch.sum(input=dy_v[:, :, None] * jacobian_f, dim=(0, 1)) / (self.config['v_shot'] * self.config['num_ways']) # (X, )

            # calculate the factorization of Gauss-Newton matrix for each data-point
            # G = B.T @ B
            # gauss_newton_factorized = B
            gauss_newton_factorized = torch.transpose(input=L_sigma, dim0=-2, dim1=-1) @ jacobian_f # (num_samples, C, X)

            # set jacobian_f to none to save GPU memory
            jacobian_f = None

            # diagonal of Gauss-Newton matrix for each data-point
            gauss_newton_diagonal = torch.sum(input=torch.square(input=gauss_newton_factorized), dim=1) # (num_samples, X)

            # reset gauss_newton_factorized to save GPU memory
            gauss_newton_factorized = None

            # diagonal of Gauss-Newton matrix for a task
            gauss_newton_diagonals[task_id, :] = torch.mean(input=gauss_newton_diagonal, dim=0) # (X, )

            # reset to none to save GPU memory
            gauss_newton_diagonal = None

        return jacobian, gauss_newton_diagonals

    def get_dynamics_cost_adam(self, xhat_t: torch.Tensor, optimizer_sd: dict, uhat_t: torch.Tensor, minibatch: typing.List[typing.Dict[str, torch.Tensor]]) -> typing.Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        jacobian, gauss_newton_diagonals = self.jacobian_gauss_newton_matrices(xhat_t=xhat_t, minibatch=minibatch)
        ...
        Cuu = self.config['beta_u'] * torch.eye(n=self.config['minibatch'], device=self.config['device'])
        ...
        if len(optimizer_sd['state']) > 0:
            ...
            for param_id in range(len(optimizer_sd['state'])):
                ...
                m_t_1[pointer:(pointer + num_param)] = torch.flatten(input=optimizer_sd['state'][param_id]['exp_avg'])[:, None]
                v_t_1[pointer:(pointer + num_param)] = torch.flatten(input=optimizer_sd['state'][param_id]['exp_avg_sq'])[:, None]
    ...
    def LQR_backward(self, xhats: torch.Tensor, opt_sds: typing.List[dict], uhats: typing.Union[np.ndarray, torch.Tensor], minibatches: typing.List[typing.Dict[str, torch.Tensor]]) -> typing.Tuple[torch.Tensor, torch.Tensor]:
        ...
        K = torch.empty(size=(len(minibatches), uhats.shape[1], xhats.shape[1]), device=self.config['device'])
        k = torch.empty(size=(len(minibatches), uhats.shape[1], 1), device=self.config['device'])
    ...
    def LQR_forward(self, xhats: torch.Tensor, optimizer_sd: dict, uhats: torch.Tensor, K: torch.Tensor, k: torch.Tensor, minibatches: typing.List[typing.Dict[str, torch.Tensor]], alpha: float) -> typing.Tuple[torch.Tensor, torch.Tensor, typing.List[dict], np.ndarray, torch.Tensor]:
        ...
        for t in range(len(minibatches)):
            ...
            params = vector_to_list_parameters(vec=x[t, :], parameter_shapes=self.parameter_shapes)
            # enable gradient
            for param in params:
    ...
    def iLQR(self, xhats: torch.Tensor, opt_sds: typing.List[dict], uhats: typing.Union[np.ndarray, torch.Tensor], minibatches: typing.List[typing.Dict[str, torch.Tensor]], num_iters: int, total_cost_hat: float) -> typing.Tuple[torch.Tensor, torch.Tensor, typing.List[dict], np.ndarray, torch.Tensor]:
